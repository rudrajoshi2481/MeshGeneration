"""
train_final.py — MeshGPT full training run with instrumentation.
...
# ── output dirs ──────────────────────────────────────────────────────────────
OUT_DIR   = "/data/joshi/MESHGPT/final"
...
def plot_dashboard(hist, data, epoch, step, out_dir):
    ...
    fig.suptitle(
        ...
        f"IoU={ious.mean():.4f}   Cls={cls_arr.mean()*100:.1f}%   "
...
class DashboardCallback(Callback):
    ...
    def _generate(self, model, trainer, epoch, step):
        ...
        self._metric_history["cls_acc"].append(float(cls_arr.mean()))
        ...
        if self.meta_path and os.path.exists(self.meta_path):
            try:
                ...
                meta["training"]["current_cls_acc"] = round(float(cls_arr.mean()), 4)
        ...
        print(f"[Dashboard] IoU={ious.mean():.4f}  util={util:.3f}  "
              f"cls={cls_arr.mean():.3f}  dead={dead}  → {edir}/")
        model.train()
...
def main():
    ...
    logger = CSVLogger(save_dir=os.path.join(OUT_DIR, "logs"), name="csv")
    ...
    print(f"\n{'='*60}")
    print(f"  MeshGPT Final Run — {'QUICK TEST' if args.quick else 'FULL TRAINING'}")
