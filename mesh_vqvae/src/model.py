...
class MaskedVQVAE3D(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        self.use_classifier = cfg.train.use_classifier
        ...
        self.cls_weight_max = cfg.train.cls_weight_max if self.use_classifier else 0.0
    ...
    def compute_loss(self, out: Dict, batch: Dict) -> Dict:
        ...
        if self.use_classifier and out["class_logits"] is not None:
            cls_loss = F.cross_entropy(out["class_logits"], labels)
        else:
            cls_loss = torch.tensor(0.0, device=recon_loss.device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_classifier and out["class_logits"] is not None:
            ...
            acc = torch.tensor(0.0, device=iou.device)
    ...
    def configure_optimizers(self):
        ...
        # Separate learning rates: classifier needs much higher LR (if enabled)
        if self.use_classifier:
            ...
            param_groups = [
                {"params": self.parameters(), "lr": base_lr, "name": "base"},
            ]
